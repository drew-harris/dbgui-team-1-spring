...
import { doctorOnlyMiddleware, patientOnlyMiddleware } from "../middleware/authMiddleware";
...
prescriptionRouter.post("/:id/refill", patientOnlyMiddleware, async (req, res) => {
    const { id } = req.params;
...
prescriptionRouter.put("/:id/refill/:action", doctorOnlyMiddleware, async (req, res) => {
